            ...
            <div className="grid items-start gap-6 lg:gap-20 md:grid-cols-2 lg:grid-cols-3">
               ...
               <div className="sticky md:mx-auto max-w-xl md:max-w-[24rem] grid gap-8 p-0 md:p-6 md:px-0 top-[6rem] lg:top-[8rem] xl:top-[10rem]">
                  <div className="grid gap-2">
                     ...
                     <span className="max-w-prone whitespace-pre-wrap inherit text-copy opacity-50 font-medium">
                        {product.vendor}
                     </span>
                  </div>
         ...
         <div className="grid gap-4">
            {options.map(({name, values}) => {
               ...
               return (
                  ...
                  >
                     <legend className="whitespace-pre-wrap max-w-prose font-bold text-lead min-w-w[4rem]">
                        {name}
                     </legend>
      ...
      <>
         ...
         {isOutOfStock ? (
            ...
            <BuyNowButton variantId={selectedVariant.id}>
               <span className="inline-block rounded-sm font-medium text-center py-3 px-6 max-w-xl leanding-none border w-full">
                  Buy it now
               </span>
            </BuyNowButton>
         )}
         ...
         {values.map(value => {
            ...
            return (
               ...
               >
                  ...
                  />
                  <div className={`leading-none border-b-[2px] py-1 cursor-pointer transition-all duration-200 ${checked ? "border-gray-500" :"border-neutral-50"}`}>
                     {value}
                  </div>
               </label>
            )
...
const ProductGallery = ({ media }) => {
   ...
   return (
      <div className={`grid gap-4 overflow-x-hidden grid-flow-col md:grid-flow-row md:p-0 md:overflow-x-auto md:grid-cols-2 w-screen md:w-full lg:col-span-2`}>
         {media.map((med, i) => {
            let extraProps = {}
